      ...
      <section class="modal-card-body">
        <div class="is-flex is-flex-direction-column">
          ...
          <div class="select is-fullwidth mt-4">
            <select v-model="chofer" :disabled="resultadoDeReq === 200">
...
export default {
  ...
  watch: {
    open(oldValue, newVaue) {
      this.placa = this.current.placa
      this.chofer = this.current.chofer
    },
  },
  methods: {
    ...
    async updateBus() {
      ...
      const key = this.current.placa
      const payload = {
        resource,
        data,
        key,
      }
      this.resultadoDeReq = await this.updateData(payload)
